"""
amr_comm_node.py - Transport & Mesh Routing Node
Manages Wi-Fi vs. Wi-SUN dual interface, throttling, duplicate suppression, and gateway bridging.
"""
import time
...
class AMRCommNode:
    ...
    def process_inbox(self, now: float) -> list[dict]:
        ...
        for pkt in wisun_pkts:
            ...
            # BRIDGE LOGIC: If I am in Wi-Fi zone and received a Wi-SUN status from a Dead Zone AMR,
            # relay it to the Server via Wi-Fi.
    ...
    def handle_task_broadcast(self, task_pkt: LinkPacket, active_deadzone_robots: list[int], now: float):
        """Designated Forwarder rule: Lowest ID Wi-Fi robot bridges server tasks to Wi-SUN."""
        if self.in_dead_zone or not active_deadzone_robots:
            return

        # Find available Wi-Fi bridge candidates (robots NOT in dead zone)
        wifi_active_nodes = [r_id for r_id, iface in self.peer_interfaces.items() if iface == 'WIFI']
        wifi_active_nodes.append(self.robot_id)

        # Lowest ID acts as designated Wi-SUN bridge
        designated_forwarder = min(wifi_active_nodes)
