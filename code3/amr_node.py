...
class AMRCommNode:
    ...
    def process_queue(self, now: float):
        ...
        while self.queue and processed < process_limit:
            ...
            if self.in_dead_zone:
                ...
                # If sending to a known dead-zone node, we might need to route through Wi-SUN
                if pkt.destination_id != BROADCAST and self.peer_interfaces.get(pkt.destination_id) == 'WISUN':
                    pkt.medium = "WISUN"
                    self.wisun_net.send(pkt, now)
                else:
                    pkt.medium = "WIFI"
    ...
    def _handle_received_packet(self, pkt: LinkPacket, now: float):
        ...
        # Forwarding logic
        if pkt.destination_id != self.robot_id:
            import copy
            ...
            # Bridge logic: if received on WISUN, and we are on WIFI, forward via WIFI
            if not self.in_dead_zone and pkt.medium == "WISUN":
                # We are acting as a bridge!
                if self.on_event:
